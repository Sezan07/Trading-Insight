...
function MarketSegments() {
  ...
  const [usd_inrPercentageChange, setusd_inrPercentageChange] = useState('');
  const [usd_inrCurrent, setusd_inrCurrent] = useState('');
  ...
  useEffect(() => {
    const fetchDataAndRenderCharts = async () => {
      ...
      console.log(nifty50PercentageChangeData.percentage_change.toFixed(2));
      ...
      const usd_inrPercentageChangeResponse = await fetch(`http://127.0.0.1:5000/stock_pc?stock_name=${'INR=X'}`);
      const usd_inrPercentageChangeData = await usd_inrPercentageChangeResponse.json();
      setusd_inrPercentageChange(usd_inrPercentageChangeData.percentage_change.toFixed(2)); // Rounding to 2 decimal places
        
      const usd_inrCurrentResponse = await fetch(`http://127.0.0.1:5000/current_p?stock_name=${'INR=X'}`);
      const usd_inrCurrentData = await usd_inrCurrentResponse.json();
      setusd_inrCurrent(usd_inrCurrentData.current_price.toFixed(2));
      console.log(nifty50PercentageChange);
      ...
      const usd_inrResponse = await fetch(`http://127.0.0.1:5000/stock_d?stock_name=${'INR=X'}`);
      const usd_inrData = await usd_inrResponse.json();

      if (!usd_inrResponse.ok) {
        throw new Error('Failed to fetch usd_inr data');
      }
    ...
    new Chart(stockCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: nifty50PercentageChangeData.percentage_change.toFixed(2) >= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
              ...
              data: {
                ...
                datasets: [{
                 // label: 'Stock Price',
                  ...
                  borderColor: bankniftyPercentageChangeData.percentage_change.toFixed(2) >= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
                  backgroundColor: 'transparent', // Remove area fill
                  tension: 0.2,
                  fill: false,
                  borderWidth: 2,
                ...
                data: {
                  ...
                  datasets: [{
                   // label: 'Stock Price',
                    ...
                    borderColor: sensexPercentageChangeData.percentage_change.toFixed(2) >= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
                    backgroundColor: 'transparent', // Remove area fill
                    tension: 0.2,
                    fill: false,
                    borderWidth: 2,
                  ...
                  data: {
                    ...
                    datasets: [{
                     // label: 'Stock Price',
                      ...
                      borderColor: finniftyPercentageChangeData.percentage_change.toFixed(2)>= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
                      backgroundColor: 'transparent', // Remove area fill
                      tension: 0.2,
                      fill: false,
                      borderWidth: 2,
                ...
                if (stockChartRef5.current) {
                    ...
                    data: {
                      ...
                      datasets: [{
                       // label: 'Stock Price',
                        ...
                        borderColor: niftyitPercentageChangeData.percentage_change.toFixed(2)>= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
                        backgroundColor: 'transparent', // Remove area fill
                        tension: 0.2,
                        fill: false,
                        borderWidth: 2,
                  ...
                  if (stockChartRef6.current) {
                    ...
                    new Chart(stockCtx, {
                      type: 'line',
                      data: {
                        labels: usd_inrData.map(point => point.x),
                        datasets: [{
                         // label: 'Stock Price',
                          data: usd_inrData.map(point => point.y),
                          borderColor: usd_inrPercentageChangeData.percentage_change.toFixed(2) >= 0 ? '#198639' : 'rgb(212, 25, 40)', // Change border color dynamically
                          backgroundColor: 'transparent', // Remove area fill
                          tension: 0.2,
                          fill: false,
                          borderWidth: 2,
  ...
  return (
    <div>
      ...
      <div className="flex items-center mb-10">
        ...
        <div className="mr-2">
          <p className="text-lg">{usd_inrCurrent}</p>
          <p className="text-lg" style={getPercentageChangeStyle(usd_inrPercentageChange)}>({usd_inrPercentageChange}%)</p>
        </div>
      </div>
